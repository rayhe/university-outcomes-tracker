...
recs = [r for r in d["universities"] if "_endowment_source" not in r]
...
for r in recs:
    ...
    time.sleep(0.6)
    # incremental checkpoint: survives a restart mid-run (v0.20 lesson)
    json.dump(out, open(os.path.join(RAW, "infobox_extract_partial.json"), "w"), indent=1)
