...
function showDetail(id){
  ...
  const synthGray = ' <span style="font-size:.75rem;color:#9aa0b8">'; // shared synthetic-source suffix, mirrors researchLabel wording
  const empLabel = (u._employment_6mo_source && u._employment_6mo_source.startsWith('synthetic')) ? 'synthetic placeholder' : (u._employment_6mo_source || 'unlabeled');
  const givLabel = (u._alumni_giving_source && u._alumni_giving_source.startsWith('synthetic')) ? 'synthetic placeholder' : (u._alumni_giving_source || 'unlabeled');
  ...
  <div><b>Public Filings</b><br>IPEDS: ${u.filings.ipeds}<br>IRS 990: ${u.filings['990']}<br>Audited: ${u.filings.audited}<br>Scorecard: ${u.filings.scorecard} ${u.scorecard_id?`→ <a href="https://collegescorecard.ed.gov/school/?${u.scorecard_id}" target="_blank">${u.scorecard_id}</a>`:''}<br>HERD: ${u.filings.herd}<br>State Audit: ${u.filings.state_audit}<br><br><span style="font-size:.75rem;color:#9aa0b8">Research Spend: $${u.research_spend_m}M (${researchLabel}) • Conf ${u.conference}</span></div>
  </div>
  ${provenance}
